...
class DataLoadHandler {
  ...
  factory DataLoadHandler({BuildContext? context}) =>
      _instance ??= DataLoadHandler._(context!);

  int criticalId =
      0; // for handling processes entering the loadDataAndCheckForUpdate function!

  //stores:---------------------------------------------------------------------
  late DataStore _dataStore = Provider.of<DataStore>(context, listen: false);
  late TechnicalNameWithTranslationsStore _technicalNameWithTranslationsStore =
      Provider.of<TechnicalNameWithTranslationsStore>(context, listen: false);
  late UpdatedAtTimesStore _updatedAtTimesStore =
      Provider.of<UpdatedAtTimesStore>(context, listen: false);
  ...
  Future<bool> hasNoLocalData() async =>
      await _dataStore.isDataSourceEmpty() ||
      await _technicalNameWithTranslationsStore.isDataSourceEmpty();

  Future<bool> answerWasUpdated() async =>
      await _appSettingsStore.getAnswerWasUpdated() ?? false;

  void showErrorMessage(
      {required Widget messageWidgetObserver,
      String? buttonLabel,
      required var onPressedButton,
      Duration? duration}) {
    ScaffoldMessenger.of(context).clearSnackBars();
    buttonLabel ??= _technicalNameWithTranslationsStore
        .getTranslationByTechnicalName(LangKeys.no_internet_button_text);
  ...
  Future<Map<String, bool>> updatedAtWasChanged() async {
    ...
    if (DateTime.parse(oldUpdatedAtTimes.last_apps_request_time).isBefore(
      ...
      UpdatedAtTimes newUpdatedAtTimes =
          await _updatedAtTimesStore.getUpdatedAtTimesFromApi();

      return {
        UpdatedAtTimesFactory.LAST_UPDATED_AT_CONTENT:
            DateTime.parse(oldUpdatedAtTimes.last_updated_at_content).isBefore(
                DateTime.parse(newUpdatedAtTimes.last_updated_at_content)),
        UpdatedAtTimesFactory.LAST_UPDATED_AT_TECHNICAL_NAMES:
            DateTime.parse(oldUpdatedAtTimes.last_updated_at_technical_names)
                .isBefore(DateTime.parse(
                    newUpdatedAtTimes.last_updated_at_technical_names))
      };
    } else {
      return {
  ...
  Future loadDataAndCheckForUpdate(
      {bool initialLoading = false,
      bool refreshData = false,
      int processId = 0}) async {
      ...
      if (await hasInternet()) {
        ...
        await updatedAtWasChanged().then((updatedAtTimesUpdatedMap) async => {
              if (updatedAtTimesUpdatedMap.length > 0 &&
                  updatedAtTimesUpdatedMap.values
                      .any((updateAtChanged) => updateAtChanged))
                {
                  await loadDataFromApi(
                      updatedAtTimesUpdatedMap[UpdatedAtTimesFactory
                          .LAST_UPDATED_AT_TECHNICAL_NAMES]!,
                      updatedAtTimesUpdatedMap[
                          UpdatedAtTimesFactory.LAST_UPDATED_AT_CONTENT]!)
                }
            });
        _dataStore.loadingFinished();
  ...
  Future<bool> checkInternetConnectionAndShowMessage(
      {int processId = 0}) async {
  ...
  void showExceptionMessageWithBackgroundCheck(
      {String? text,
      required String backupText,
      int? durationInMilliseconds,
      int processId = 0}) async {
    ScaffoldMessenger.of(context).clearSnackBars();
    String text = _technicalNameWithTranslationsStore
        .getTranslationByTechnicalName(LangKeys.no_internet_message);
    showErrorMessage(
        duration: durationInMilliseconds != null
  ...
  void showServerErrorMessage() {
    showExceptionMessageWithBackgroundCheck(
      text: _technicalNameWithTranslationsStore
          .getTranslationByTechnicalName(LangKeys.cant_reach_server),
  ...
  void showUpdateRequiredMessage() {
    showErrorMessage(
        ...
        messageWidgetObserver:
            Text(NecessaryStrings.update_is_necessary_message_text),
        onPressedButton: () {
  ...
  loadDataFromDb() async {
    await _technicalNameWithTranslationsStore
        .getTechnicalNameWithTranslationsFromDb();
  ...
  loadDataFromApi(
    ...
    if (technicalNamesShouldBeUpdated &&
      ...
      await _technicalNameWithTranslationsStore
          .getTechnicalNameWithTranslationsFromApi();
    }
